  ...
  try {
    ...
    if (dbConnected && db) {
      ...
      if (req.file) {
        ...
        insertData.image_url = `data:${mimeType};base64,${base64Image}`;
      }
      
      const result = await db.insert(categoriesTable).values(insertData).returning();
  ...
  try {
    ...
    if (dbConnected && db) {
      ...
      if (req.files && req.files.length > 0) {
        ...
        if (IS_NETLIFY) {
          try {
            // On Netlify, check file size and convert to base64
            const maxSize = 2 * 1024 * 1024; // 2MB limit
            
            if (file.buffer.length > maxSize) {
              console.log('❌ Image too large, skipping upload');
              imageUrl = null;
            } else {
  ...
  try {
    ...
    if (dbConnected && db) {
      ...
      if (req.file) {
        const base64Image = req.file.buffer.toString('base64');
        const mimeType = req.file.mimetype;
        updateData.image_url = `data:${mimeType};base64,${base64Image}`;
      }
...
app.put('/api/admin/products/:id', authenticateAdmin, upload.array('images', 10), async (req, res) => {
  try {
    ...
    if (dbConnected && db) {
      ...
      if (req.files && req.files.length > 0) {
        const file = req.files[0];
        console.log('📁 Processing update file:', file.originalname, 'Size:', file.buffer.length);
        ...
        if (IS_NETLIFY) {
          try {
            ...
            if (file.buffer.length > maxSize) {
              console.log('❌ Image too large, skipping upload');
              // Don't update image_url if file too large
            } else {
              const base64Image = file.buffer.toString('base64');
              const mimeType = file.mimetype || 'image/jpeg';
              updateData.image_url = `data:${mimeType};base64,${base64Image}`;
            ...
            console.error('❌ Image processing error:', error);
            // Don't update image_url if processing fails
          }
        } else {
          const filename = `${Date.now()}-${file.originalname}`;
          const uploadPath = path.join(__dirname, 'uploads', filename);
          ...
          console.log('💾 Image saved locally:', updateData.image_url);
        }
      } else {
