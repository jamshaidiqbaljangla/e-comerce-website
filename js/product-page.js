...
const ProductPage = {
    init: function() {
        // Get product ID from URL parameter first, then fallback to data attribute
        ...
        if (!productId) {
            ...
            productId = productDetailSection.getAttribute('data-product-id');
            if (!productId) {
        ...
        // Update the data attribute with the correct product ID
        const productDetailSection = document.querySelector('.product-detail');
        if (productDetailSection) {
  ...
  mainImage.src = (product.images && product.images.primary)
    ...
    : (product.image_url || mainImage.src);
        ...
        if (mainImageSlides.length > 0 && product.images && product.images.gallery && product.images.gallery.length > 0) {
        ...
        } else if (mainImageSlides.length > 0) {
            // Fallback: use the primary image for all slides if no gallery exists
            const primaryImage = (product.images && product.images.primary) || product.image_url || 'images/placeholder.jpg';
            mainImageSlides.forEach(slide => slide.src = primaryImage);
            thumbnails.forEach(thumb => thumb.src = primaryImage);
        }
