...
debugLog.log('main', 'Script starting to execute...');

const app = {
    init: function() {
        debugLog.log('main', 'app.init() called.');
        ...
        setTimeout(() => {
            ...
            if (typeof window.PRODUCTS !== 'undefined') {
                debugLog.log('main', 'PRODUCTS object found. Initializing ProductRenderer and CartManager...');
                
                if (typeof window.ProductRenderer !== 'undefined' && typeof window.ProductRenderer.init === 'function') {
                    debugLog.log('main', 'Initializing ProductRenderer...');
                    window.ProductRenderer.init();
                } else {
                    debugLog.error('main', 'ProductRenderer is NOT defined or ProductRenderer.init is not a function.');
                }

                if (typeof window.CartManager !== 'undefined' && typeof window.CartManager.init === 'function') {
                    window.CartManager.init();
                } else {
                    debugLog.error('main', 'CartManager is NOT defined or CartManager.init is not a function.');
                }
                // In the app.init function
                if (typeof window.ShopManager !== 'undefined') {
                    window.ShopManager.init();
                } else {
                    debugLog.error('main', 'ShopManager is NOT defined or ShopManager.init is not a function.');
                }
...
if (window.COLLECTIONS_API && typeof window.COLLECTIONS_API.loadCollections === 'function') {
    console.log('[DEBUG] main.js: Initializing COLLECTIONS API...');
    window.COLLECTIONS_API.loadCollections();
}
                // In the app.init function
        ...
        debugLog.log('main', 'app.init() finished.');
    },
    ...
    addProductIDsToStaticCards: function() {
        ...
        debugLog.log('main', `Found ${staticCards.length} static product cards without IDs`);
        ...
        tabs.forEach(tab => {
            tab.addEventListener('click', function() {
                ...
                tabContents.forEach(content => {
                    // CSS handles display based on .active class
                    if (content.id === tabId) {
                ...
                if (typeof window.ProductRenderer !== 'undefined' && typeof window.ProductRenderer.loadTabContent === 'function') {
                    ...
                    if (tabId !== 'trending' && tabContentElement && tabContentElement.children.length === 0) {
                        const trendingContentElement = document.getElementById('trending');
                        if(trendingContentElement) {
                            // Safe DOM cloning instead of innerHTML
                            const clonedContent = trendingContentElement.cloneNode(true);
                            tabContentElement.appendChild(clonedContent);
...
if (window.CATEGORIES_API && typeof window.CATEGORIES_API.loadCategories === 'function') {
    console.log('[DEBUG] main.js: Initializing CATEGORIES API...');
    try {
        const categories = await window.CATEGORIES_API.loadCategories();
        if (categories) {
            console.log('[DEBUG] main.js: CATEGORIES API initialized successfully');
            window.CATEGORIES = categories; // Make categories available globally
        } else {
