...
setupCategories: async function() {
    if (window.CATEGORIES && window.CATEGORIES.getAllCategories) {
        console.log('[DEBUG] category-manager.js: Loading categories from CATEGORIES API...');
        
        // Get all categories from API
        const allCategories = window.CATEGORIES.getAllCategories();
        ...
        allCategories.forEach(category => {
            ...
            // Also allow access by slug for backward compatibility
            if (category.slug) {
                this.state.categories[category.slug] = this.state.categories[category.id];
            }
        });
        
        console.log('[DEBUG] category-manager.js: Loaded', allCategories.length, 'categories from API');
        
        // Find related categories
        this.findRelatedCategories();
    } else {
        console.warn('[DEBUG] category-manager.js: CATEGORIES API not available, using fallback data');
        // Keep existing categoryData as fallback
        this.state.categories = { ...this.categoryData };
        
        // If PRODUCTS is available, enhance with product counts
        if (window.PRODUCTS && Array.isArray(window.PRODUCTS.items)) {
            const categoryCounts = {};
            
            window.PRODUCTS.items.forEach(product => {
            ...
            Object.keys(categoryCounts).forEach(categoryId => {
                if (!this.state.categories[categoryId]) {
                    this.state.categories[categoryId] = {
                        ...
                        description: `Explore our ${this.formatCategoryName(categoryId)} collection.`,
                        image: 'images/category-default.jpg'
                    };
                }
                
                this.state.categories[categoryId].count = categoryCounts[categoryId];
            });
